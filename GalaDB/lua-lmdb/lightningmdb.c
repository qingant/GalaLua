...
static void _register(lua_State* L, const char *name, const struct luaL_Reg *flist)
{
	 luaL_newmetatable(L,name);
	 //lua_set_funcs(L,ENV,env_methods);
	 luaL_register(L, NULL, flist);
...
void env_register(lua_State* L) {
    ...
    /* //lua_set_funcs(L,ENV,env_methods); */
	/* lua_register(L, NULL, env_methods); */
...
void cursor_register(lua_State* L) {
	...
	_register(L, CURSOR, cursor_methods);
}
