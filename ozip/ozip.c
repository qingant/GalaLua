...
int ozip_zipOpenNewFileInZip4_64(zipFile file, const char* filename,
		const zip_fileinfo* zipfi, const void* extrafield_local,
		uint32_t size_extrafield_local, const void* extrafield_global,
		uint32_t size_extrafield_global, const char* comment, int method,
		int level, int raw, int windowBits, int memLevel, int strategy,
		const char* password, uint32_t crcForCrypting, uint32_t versionMadeBy,
		uint32_t flagBase, int zip64) {
	...
	return zipOpenNewFileInZip4_64(file, filename, zipfi, extrafield_local,
			...
			password, crcForCrypting, versionMadeBy, flagBase, zip64);
}
...
int ozip_zipCloseFileInZipRaw64(zipFile file, uint64_t uncompressed_size,
		uint32_t crc32) {
...
/* unzip API */
int ozip_unzStringFileNameCompare(const char* fileName1, const char* fileName2,
...
int ozip_unzLocateFile(unzFile file, const char *szFileName,
		int iCaseSensitivity) {
...
int ozip_unzGetCurrentFileInfo64(unzFile file, unz_file_info64 *pfile_info,
		char *szFileName, uint32_t fileNameBufferSize, void *extraField,
		uint32_t extraFieldBufferSize, char *szComment,
		uint32_t commentBufferSize) {
	return unzGetCurrentFileInfo64(file, pfile_info, szFileName,
...
int ozip_unzOpenCurrentFile3(unzFile file, int* method, int* level, int raw,
		const char* password) {
