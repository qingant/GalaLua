...
local checkeof = check_function(function(ret) return ret == 0 or ret == ozip.UNZ_END_OF_LIST_OF_FILE end)
...
function fs_unz.stringFileNameCmp(self,fname1, fname2, icase)
	return self._archive.strFileNameCmp(fname1, fname2, icase)
end
...
function fs_unz.getGlobalInfo(self)
	return self._archive:getGlobalInfo()
end

function fs_unz.getGlobalComment(self, size)
	return self._archive:getGlobalComment(size)
end

function fs_unz.goToFirstFile(self)
	return self._archive:goToFirstFile()
end

function fs_unz.goToNextFile(self)
	return self._archive:goToNextFile()
end

function fs_unz.locateFile(self, fname, icase)
	return self._archive:locateFile(fname, icase)
end

function fs_unz.getFilePos(self )
	return self._archive:getFilePos()
end

function fs_unz.goToFilePos(self, pos_in_dir, num_of_file )
	return self._archive:goToFilePos()
end

function fs_unz.info(self, fname_size, extra_size, comment_size )
	return self._archive:getCurrentFileInfo(fname_size, extra_size, comment_size)
end

function fs_unz.getCurrentFileZStreamPos(self )
	return self._archive:getCurrendFileInfoZStreamPos()
end

function fs_unz.openFile(self, raw, password )
	return self._archive:openFile(raw, password)
end

function fs_unz.closeFile(self)
	return self._archive:closeFile()
end

function fs_unz.readFile( self, size )
	return self._archive:readFile(size)
end

function fs_unz.tell(self)
	return self._archive:tell()
end

function fs_unz.eof(self )
	return self._archive:eof()
end

function fs_unz.getLocalExtraField(self)
	return self._archive:getLocalExtraField()
end

function fs_unz.getOffset(self)
	return self._archive:getOffset()
end

function fs_unz.setOffset(self, offset )
	return self._archive:setOffset64(offset)
end
