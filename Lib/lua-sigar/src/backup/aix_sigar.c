...
int sigar_proc_owner_get(uid_t id,char *out)
{
    if(id < 0 ||NULL == out){
        return -1;
    }
    char *name= IDtouser(id);
    if(NULL != name){
                        ...
                        sigar_proc_time_t *proctime)
{
    ...
    proctime->user = pinfo->pi_ru.ru_utime.tv_sec * SIGAR_MSEC+pinfo->pi_ru.ru_utime.tv_usec/SIGAR_MSEC;
    proctime->sys  = pinfo->pi_ru.ru_stime.tv_sec * SIGAR_MSEC+pinfo->pi_ru.ru_stime.tv_usec/SIGAR_MSEC;
